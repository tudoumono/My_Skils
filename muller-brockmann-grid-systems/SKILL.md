...
description: 本物のミュラー＝ブロックマン式モジュラーグリッド（インターナショナル・タイポグラフィック・スタイル）で編集/雑誌/レポートWebページを構築。規律（カラム＋モジュール＋ベースライン、グロテスク書体、左揃え、抑制された黒/白/赤）と、グリッドを実体化・可視化・検証するフロントエンド技術を内蔵。スキャフォルド生成器とPuppeteer検証ハーネス付き。
metadata:
  short-description: ミュラー＝ブロックマン式グリッド
---
